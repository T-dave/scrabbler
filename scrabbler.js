...
for (let i = 0; i < newWords.length; i++) {
	...
	for (let i = 0; i < newWords.length; i++) {
	for (let j = 0; j < fiveFive.length; j++) {
	if (fiveSix[j] === newWords[i].slice(1)) {
		fiveSix.splice(j, 1);
	}}
	fiveSix.push(newWords[i].slice(1));
	for (let j = 0; j < fiveFive.length; j++) {
	if (fiveFive[j] === newWords[i].slice(2)) {
		fiveFive.splice(j, 1);
	}}
	fiveFive.push(newWords[i].slice(2));
	for (let j = 0; j < fiveFour.length; j++) {
	if (fiveFour[j] === newWords[i].slice(3)) {
		fiveFour.splice(j, 1);
	}}
	fiveFour.push(newWords[i].slice(3));
	for (let j = 0; j < fiveThree.length; j++) {
	if (fiveThree[j] === newWords[i].slice(4)) {
		fiveThree.splice(j, 1);
	}}
	fiveThree.push(newWords[i].slice(4));
	for (let j = 0; j < fiveTwo.length; j++) {
	if (fiveTwo[j] === newWords[i].slice(5)) {
		fiveTwo.splice(j, 1);
	}}
	fiveTwo.push(newWords[i].slice(5));
}	
}else if(wordLength ===8){
	scrabbleEight();
}else if(wordLength === 9){
...
if (wordLength === 0) {
	...
	h3.innerHTML = 'Scrabbled words are: <br>' + result.join('<br> ') + 'Word too long for total scrabbling';
}
